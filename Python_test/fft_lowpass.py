...
with open("dennis_raw.txt") as f:
    str_img_test = f.read()

img_test = np.array(str_img_test.split(",")[:-1], dtype=np.uint8)
img_test = img_test.reshape(240, 320)
stacked_img = np.stack((img_test,)*3, axis=-1)
...
# TFcircleIN   = draw_cicle(shape=stacked_img.shape[:2],diamiter=50)
...
# fft_img = np.zeros_like(stacked_img,dtype=complex)
# for ichannel in range(fft_img.shape[2]):
#     fft_img[:,:,ichannel] = np.fft.fftshift(np.fft.fft2(stacked_img[:,:,ichannel]))
